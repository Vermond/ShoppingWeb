...
import type { AdminReportResponse } from './admin-reports.types';
...
export class AdminReportsController {
  ...
  @Get()
  @ApiOperation({ summary: '관리자 리포트 조회' })
  @ApiQuery({
    name: 'from',
    required: false,
    example: '2026-08-01',
  ...
  async findReports(@Query() query: unknown): Promise<AdminReportResponse> {
    const period = parseAdminDashboardQuery(query);

    return this.adminReportsService.findReports(period);
  }
}
